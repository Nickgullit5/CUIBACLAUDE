...
const NavComponent = ({ accent = '#111110' }) => {
    ...
    <nav style={{
      ...
      <div style={{ display:'flex', alignItems:'center', gap:14, cursor:'pointer' }} onClick={() => window.scrollTo({top:0, behavior:'smooth'})}>
        <img src="uploads/logo-cuiba-light.png" alt="Cuiba Design" style={{
          height: 52, width: 'auto', filter: logoFilter, transition: 'filter 0.4s ease'
        }} />
        <span style={{
          fontFamily:'Montserrat,sans-serif', fontSize:17, fontWeight:700,
      ...
      {/* Desktop links */}
      <div className="desktop-links" style={{display:'flex', alignItems:'center', gap: 40}}>
        ...
        <button onClick={() => scrollTo('funnel')} style={{
          background: accent, color:'#F9F8F5', border:'none', cursor:'pointer',
